...
public class UsersServiceImp implements UsersService {
    ...
    public SignUpResponseDto signUp(SignUpRequestDto form) {
        ...
        UserModel user = userModelRepository.findByEmail(form.getEmail()).orElse(null);
    
        if(user != null){
            throw new ExistingEmailException("이미 가입된 이메일입니다");
    ...
    public LoginResponseDto login(LoginRequestDto form) {
        ...
        UserModel user = userModelRepository.findByEmail(form.getEmail()).orElseThrow(()-> new UserAuthException("아이디와 비밀번호를 확인해주세요"));
